@php
    ...
    foreach ($orders as $order) {
        foreach ($order->many_cart as $cart) {
            ...
            foreach ($ids as $id) {
                if ($id) $cartAttrIds[] = $id;
            }
        }
    }
    $cartAttrIds = array_unique($cartAttrIds);
    $preloadedAtrItems = [];
    if (!empty($cartAttrIds)) {
        $preloadedAtrItems = \App\Models\AtrItem::with('productAttr')
            ->whereIn('id', $cartAttrIds)
            ->get()
            ->keyBy('id');
    }
@endphp
    ...
    <table class="table mg-b-0 table-bordered table-striped">
        ...
        <tbody>
            @foreach ($orders as $key => $order)
                @php
                    $check_duplicate = $phoneCounts[$order->phone] ?? 0;
                @endphp
                <tr>
                    <th scope="row">
                    ...
                    <td class="position-relative {{ $check_duplicate > 1 ? 'bg-danger-light' : '' }}"
                        style="vertical-align:top;">
                        @if ($order->phone)
                            <div
                                style="position:absolute; top:5px; right:5px; display:flex; flex-direction:column; gap:4px; z-index:10;">
                                <!-- WhatsApp -->
                                @php
                                    // Remove non-digits
                ...
                <img src="https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/imo-icon.png" style="width:18px;height:18px;">
                        ...
                        <p class="mb-0">
                            <a
                                href="tel:{{ $order->phone ?? 'N/A' }}"><strong>{{ $order->phone ?? 'N/A' }}</strong></a>
                        </p>
                    ...
                    <td>
                        ...
                        <div class="d-flex g-2">
                            <a href="javascript:void(0);" data-id="{{ $order->id }}"
                                class="mr-2 btn qc_btn_modal btn-icon">
                    ...
                    <td>
                        ...
                        @foreach ($order->many_cart as $cart)
                            @if ($cart->product)
                                ...
                                @endphp
                                @foreach ($ids as $id)
                                    @if(isset($preloadedAtrItems[$id]))
                                        @php $atr_item = $preloadedAtrItems[$id]; @endphp
                        ...
                        <div class="btn-group">
                            @if ($order->status == 1)
                                <button type="button" class="btn btn-primary btn-sm dropdown-toggle"
                                    ...
                                    aria-expanded="false">Processing</button>
                            @elseif($order->status == 2)
                                <button type="button" class="btn btn-warning btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Courier
                                    Entry</button>
                            @elseif($order->status == 16)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Total
                                    Courier</button>
                            @elseif($order->status == 3)
                                <button type="button" class="btn btn-warning btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> On Hold</button>
                            @elseif($order->status == 4)
                                <button type="button" class="btn btn-danger btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Cancel</button>
                            ...
                            @elseif($order->status == 7)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">on
                                    delivery</button>
                            @elseif($order->status == 7)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">on
                            ...
                            @elseif($order->status == 8)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">No Response
                                    1</button>
                            ...
                            @elseif($order->status == 17)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Printed
                                    Invoice</button>
                            ...
                            @elseif($order->status == 12)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Return</button>
                            ...
                            @elseif($order->status == 13)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Partial
                                    Delivery</button>
                            @elseif($order->status == 14)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Paid
                                    Return</button>
                            @elseif($order->status == 15)
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Stock
                                    Out</button>
