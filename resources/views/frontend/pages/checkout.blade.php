            ...
            <div class="col-12">
                <div class="tab-nav justify-content-center">
                    <button class="single-nav active" data-tab="checkOutTab" disabled>
                        <span class="txt btn-bangla">অর্ডারটি কনফার্ম করতে আপনার নাম, ঠিকানা, মোবাইল নাম্বার, লিখে অর্ডার
                            কনফার্ম করুন বাটনে ক্লিক করুন</span>
                        ...
                        <div class="gap-1 row">
                            <div class="col-xl-5 col-lg-5 col-md-5" style="border: 1px dashed #bdbdbd52;padding: 20px;">
                                <div class="billing-details">
                                    <form action="{{ route('order') }}" method="POST" id="checkout_form"
                                        class="form-row checkout_form">
                                        @csrf
                                        ...
                                        <div class="form-col-5">
                                            ...
                                            <input type="tel" class="form-control" id="phone" name="phone"
                                                pattern="^(?:\+?88)?01[13-9]\d{8}$" onkeyup="checkScore()"
                                                placeholder="অবশ্যই ১১ অক্ষর হবে" required>
                                                ...
                                                <select name="shipping_method" id="shipping_method" class="form-control"
                                                    ...
                                                    @foreach (Cart::content() as $cart)
                                                        ...
                                                        @elseif($prd && $prd->shipping == '0')
                                                            <option value="{{ $prd->outside }}"
                                                                data-amount="{{ $prd->outside }}">ঢাকার বাইরে </option>
                                                            <option value="{{ $prd->inside }}"
                                                                data-amount="{{ $prd->inside }}">ঢাকার ভিতরে </option>
                                                        @else
                                    ...
                                    <button type="submit"
                                        ...
                                        id="conf_order_btn">অর্ডার কনফার্ম করুন <i
                                            class="fa-light fa-truck-arrow-right"></i></button>

                                    <!-- incomplete order hidden fields -->
                                    <input type="hidden" id="incomplete_token" value="{{ $incompleteToken }}">

                                    @foreach (Cart::content() as $cartItem)
                                        <input type="hidden" class="product_id" name="product_ids[]"
                                            value="{{ $cartItem->id }}">
                                        <input type="hidden" class="product_slug" name="product_slugs[]"
                                            value="{{ $cartItem->options['slug'] ?? '' }}">
                                    @endforeach
                                    ...
                                    @foreach (Cart::content() as $cartItem)
                                        <input type="hidden" name="product_color[]"
                                            value="{{ $cartItem->options['color'] ?? '' }}">
                                        <input type="hidden" name="product_size[]"
                                            value="{{ $cartItem->options['size'] ?? '' }}">
                                        <input type="hidden" name="product_model[]"
                                            value="{{ $cartItem->options['model'] ?? '' }}">
                                    @endforeach
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-6" style="border: 1px dashed #bdbdbd52;">
                                        ...
                                        <tbody>
                                            @foreach (Cart::content() as $cart)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('details', $cart->options['slug']) }}"
                                                            class="product-name">
                                                            {{ $cart->name }}
                                                        </a>
                                                    </td>
                                                    ...
                                                    <td class="cart_qty">
                                                        <div class="product-count cart-product-count">
                                                            <div class="quantity-1 rapper-quantity">
                                                                ...
                                                                <div class="qty_div_1">
                                                                    <div class="q-down">
                                                                        <i class="fa-solid fa-minus qty_minus"
                                                                            id="qty_minus{{ $cart->id }}"
                                                                            data-id="{{ $cart->rowId }}"
                                                                            data-qty="{{ $cart->qty }}"></i>
                                                                    </div>
                                                                    <div class="qty-div">
                                                                        <input type="number" name="qty"
                                                                            id="qty" min="1"
                                                                            class="qty qty{{ $cart->rowId }}"
                                                                            data-id="{{ $cart->rowId }}"
                                                                            value="{{ $cart->qty }}">
                                                                    </div>
                                                                    <div class="q-up">
                                                                        <i class="fa-solid fa-plus qty_plus"
                                                                            id="qty_plus{{ $cart->id }}"
                                                                            data-id="{{ $cart->rowId }}"
                                                                            data-qty="{{ $cart->qty }}"></i>
                                                                    </div>
                                                    ...
                                                    <td class="c_price{{ $cart->rowId }}">{{ $cart->total }}
                                                    </td>
                                                    <td><a href="{{ route('cart.destroy', $cart->rowId) }}"
                                                            class="cart-delete"><i
                            ...
                            <div class="payment-method">
                                <div class="total-clone">
                                    <ul>
                                        <li>Sub Total <span class="price-txt"
                                                id="sub_total">৳{{ Cart::subtotal() }}</span></li>
                                        <li>Shipping <span class="price-txt" id="cart_shipping_cost">0</span></li>
                                        <li class="total-price-wrap">Total <span class="price-txt"
                                                id="net_total">৳<span
                                                    id="totalPrice2">{{ Cart::total() }}</span></span></li>
                                    </ul>
                                </div>
