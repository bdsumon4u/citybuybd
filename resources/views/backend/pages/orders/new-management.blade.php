...
@section('body-content')
            ...
            <div class="px-3 py-2 row align-items-center">
                ...
                <div class="mt-2 text-center col-md-6 text-md-right mt-md-0">
                    <div class="custom-control custom-switch d-inline-block">
                        <input type="checkbox" class="custom-control-input" id="toggle_total_with_delivery"
                    ...
                    <input type="hidden" id="with_delivery_charge" value="{{ !empty($withDeliveryCharge) ? 1 : 0 }}">
                </div>
            </div>
            <div class="assign">
    ...
    <script>
        ...
        function getData(page, event) {
            ...
            $.ajax({
                ...
                .fail(function(jqXHR, ajaxOptions, thrownError) {
                    ...
                    $('.assign').html(
                        '<div class="py-4 text-center text-danger">Failed to load data. Please check server logs.</div>'
                        );
                    $('.btn-submit').prop('disabled', false);
                });
        }
        ...
        function AssignEdit(id) {
            ...
            var status_over_cod = $('#order_assign_secret_' + id).val();
            $.ajax({
                type: "POST",
                url: "{{ url('admin/order-management/assign_edit/') }}/" + id,
                data: {
                    order_assign: order_assign,
                    status_over_cod: status_over_cod,
                    _token: token
                },
                success: function(response) {
        ...
        $(document).on('click', '.qc_btn_modal', function(e) {
            ...
            $.ajax({
                ...
                $(this).closest('td').find('.qc_result').empty().append('<span class="text-primary">T:' + (
                        data.delivered + data.returned) + '</span> <br><span class="text-success">D:' +
                    data.delivered + '</span> <br><span class="text-danger">R:' + data.returned +
                    '</span>')
