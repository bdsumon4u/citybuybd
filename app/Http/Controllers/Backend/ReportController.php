...
class ReportController extends Controller
{
     ...
     public function employee_orders_search(Request $request)
    {
        ...
        if ($searchDays == 0) {
            $orders = Order::orderBy('id', 'desc')->whereDate('created_at', Carbon::today())->where('order_assign',$employee)->paginate(50);
            $total_orders = Order::whereDate('created_at', Carbon::today())->where('order_assign',$employee)->get();
        } elseif ($searchDays == 1) {
            ...
            $date_from = \Carbon\Carbon::parse($fromDate)->format('Y-m-d');
            $date_to = \Carbon\Carbon::parse($toDate)->format('Y-m-d');
    ...
    public function employee_status($employee, $status, $searchDays,$fromDate=null,$toDate=null){
        ...
        if ($searchDays == 0) {
            if($status==0){
                ...
                $orders = Order::orderBy('id', 'desc')->whereDate('created_at', Carbon::today())->where('order_assign',$employee)->where('status',$status)->paginate(50);
                $total_orders = Order::whereDate('created_at', Carbon::today())->where('order_assign',$employee)->get();
            }
        } elseif ($searchDays == 1) {
            if($status==0){
