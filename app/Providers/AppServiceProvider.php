...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        View::composer(['frontend.*', 'frontend.includes.header'], function ($view): void {
            $categories = optimize('global_categories_nav', fn () => Category::orderBy('title', 'asc')
                ->where('status', 1)
                ->with(['subcategories' => function ($q): void {
                    $q->with('childcategories');
                }])
                ->get(), 86400, ['categories', 'subcategories', 'childcategories']);
